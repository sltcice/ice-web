      ...
      if($result)	{
        if($result && mysqli_num_rows($result) > 0)	{
          ...
          if($user_data['password'] === $password){
            ...
            // echo "Login Success!";
            return array("login" => true, "user_id" => $user_data['id'], "username" => $user_data['username'], "email" => $user_data['email'], "firstname" => $user_data['firstname'], "lastname" => $user_data['lastname'], "phone" => $user_data['phone'], "profilepic" => $user_data['profilepic'], "usertype" => $user_data['usertype'], "lastlogin" => $user_data['lastlogin'], 'status'=>$user_data['status']);
            //id, username, firstname, lastname, email, phone, profilepic, title, description, organization, batch, city, unimail, website, github, linkedin, facebook, twitter, instagram, youtube, hackerrank, password, usertype, registration, lastlogin, otp, status

            // header("Location: ../index.php?page=login&login_sucess");

            
          }else{
            header("Location: ../../?page=login&error=wc");
          }
        }else{
        header("Location: ../../?page=login&error=wc");
